	...
	private func expr(_ _p: Int) throws -> ExprContext   {
		...
		do {
			...
			case 6:
				...
				if (//closure
				 { () -> Bool in
				      let testSet: Bool = _la == PinnParser.Tokens.TWODOTS.rawValue
				      return testSet
				 }()) {
					setState(185)
					try match(PinnParser.Tokens.TWODOTS.rawValue)
