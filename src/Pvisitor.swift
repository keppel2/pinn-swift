...
class Pvisitor {
    ...
    private static let builtIns
        ...
        [
            ...
            "assert": { sctx, pv, s in
                try assertPvals(s, 2)
                if try !s[0].gg().gEquivalent(s[1].gg(), pv) {
    ...
    private func callFunction(_ sctx: ParserRuleContext, _ str: String, _ s: [Pval]) throws -> Pval? {
        ...
        for (index, v) in fh.fkinds.enumerated() {
            ...
            if try !s[index].getKind().equivalent(v.k, self) {
...
//            let aeFirstk = ae.first!.getKind()
        ...
        case let sctx as PinnParser.LiteralExprContext:
            ...
            case .STRING:
                ...
                let pv = Pval(sctx, str2, self)
                rt = pv
            case .INT:
                let str = sctx.INT()!.getText()
                let x = Int(str)!
                let pv = Pval(sctx, x, self)
                rt = pv
            case .FLOAT:
...
//                rt = pv
    ...
    private func visit(_ sctx: ParserRuleContext) throws
    {
        ...
        case let sctx as PinnParser.DoubleSetContext:
            ...
            try lh.setPV(Pval(sctx, lhsv + rhsv, self))
            ...
            if sctx.RANGE() != nil {
                ...
                case .gSlice, .gArray:
                    for x in try 0..<ranger.getCount() {
                        ...
                        try key?.setPV(Pval(sctx, x, self))
                ...
                case .gMap:
                    ...
                    for mkey in keys {
                        try key?.setPV(Pval(sctx, mkey, self))
